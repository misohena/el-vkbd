...
The easiest way to adjust the overall keyboard height is to change the :height of the `vkbd-text-keyboard' face. You can also change the font parameter in `vkbd-keyboard-frame-parameters'.

To change only the key height without changing the text size on the keys, adjust
the `vkbd-text-key-line-height' variable or :height of `vkbd-text-column-separator'.

To see the relevant customization variables, type M-x customize-group vkbd-text-style.
