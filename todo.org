...
** DONE キーの高さはline-heightで設定する方が良い
CLOSED: [2025-12-07 Sun 09:17]
1行にキーが一つしか無い場合、column-separatorは挿入されないから。

column-separatorの:heightを大きくするとベースラインの下にも空きが増える。しかしline-heightでface相対指定で増やすと下は増えない。

なのでvkbd-text-key-raiseも変更しなければならない。

ついでに vkbd-text-keyboard の継承元も fixed-pitch に変えておく。
