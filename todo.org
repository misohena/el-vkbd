...
* Inbox
** TODO スクリーンショットを更新する
** TODO vkbd-global-keyboard-optionsの:typeを真面目に書く
...
** TODO text-scale-modeと同じ原理でサイズを調整できるようにする
...
追記: [[*キーテキストのセンタリングを正確にする][キーテキストのセンタリングを正確にする]]の修正によって[[elisp-function:text-scale-adjust]]で調整するとレイアウトが崩れるようになってしまった。何か回避策はあるだろうか。
