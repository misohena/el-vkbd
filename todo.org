...
バッファローカルキーボードを作りたい。 +SVG画像じゃないと実用的じゃないと思う+ (←サイズを小さくして部分的にread-onlyにすれば十分使えると思う)。 +これをやるならキーボードの状態はバッファに持たせない方が良い+ (←現状ですでにバッファには持たせないようになった)。 一つのバッファに複数のキーボードを配置する可能性があるので。
...
** TODO text-scale-modeと似た原理でサイズを調整できるようにする
...
** TODO サイズに関するオプションをメニューに追加する
うーん、faceのカスタマイズでやってくれないかな？

** TODO line-heightテキストプロパティでキーの高さを変える
...
** TODO defcustomで設定を変更したときにキーの外観を更新すべきだろうか
faceで自動的に変わる部分は良いが、セパレータの幅はじめ他のものは自動的には変わらない。調整するのに繰り返し閉じて開き直さないといけない。

** TODO キーボードバッファでスクロール出来てしまうのを抑制する
今現在はほぼ出来なくなっているけれど、それでもまだ =M-: (scroll-up)= とかやれば出来る。また、水平スクロールは制限されていないから =C-x <= や =C-x >= はできてしまう。
スクロール位置を固定して変えさせないなんて事できるの？
検出して戻す？
