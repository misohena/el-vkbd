...
** TODO レイアウトを切り替えるボタンを通常のキーとして置けるようにする :INPUT:
要するにFnキー。
...
[[*レイアウトを切り替えるボタンを通常のキーとして置けるようにする][レイアウトを切り替えるボタンを通常のキーとして置けるようにする]]ので済ますか悩むところ。
...
[[info:elisp#Quitting][Quitting]] ([[https://ayatakesi.github.io/lispref/translation_HEAD/html/Quitting.html][日本語訳]])
...
| コード                                                     | 物理C-g                               | vkbd以前 | read-eventだけ対応後 |    |
|------------------------------------------------------------+---------------------------------------+----------+----------------------+----|
| (read-event "Prompt: ")                                    | Quit                                  |        7 | Quit                 |    |
| (read-char "Prompt: ")                                     | Quit                                  |        7 | Quit                 |    |
| (read-char-exclusive "Prompt: ")                           | Quit                                  |        7 | Quit                 |    |
| (let ((echo-keystrokes 0)) (read-event "Prompt: "))        | Quit                                  |        7 | Quit                 |    |
| (let ((inhibit-quit t)) (read-event "Prompt: "))           | 7                                     |        7 | 7                    |    |
| (read-quoted-char "Prompt: ")                              | 7  (inhibit-quitが使われているから)   |        7 | 7                    |    |
|------------------------------------------------------------+---------------------------------------+----------+----------------------+----|
| (read-key-sequence "Prompt: ")                             | Quit                                  |    "\^G" | "\^G"                | × |
| (read-key-sequence-vector "Prompt: ")                      | Quit                                  |      [7] | [7]                  | × |
| (read-key "Prompt: ")                                      | 7 (echo-keystrokesが使われているから) |        7 | 7                    |    |
| (read-minibuffer "Prompt: ")                               | Quit                                  |     Quit | Quit                 |    |
...
user-dataのスケールをドラッグで調整できれば良いわけだが、面倒。
メニューで許して。
...
** DONE キー行の改行にfaceが適用されていないのを直す
CLOSED: [2025-12-07 Sun 15:13]
...
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Face-Remapping.html][Face Remapping (GNU Emacs Lisp Reference Manual)]]
...
** DONE (サイズ調整)サイズに関するオプションをメニューに追加する
CLOSED: [2025-12-07 Sun 23:59]
