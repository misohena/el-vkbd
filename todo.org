...
* Inbox
** TODO 通常のキーとしてレイアウトを切り替えるボタンを置けるようにする
** TODO タイトルバーにレイアウトを切り替えるボタンを置けるようにする
** TODO タイトルバーに並べるボタンをカスタマイズできるようにする
** TODO タイトルバーを消せるようにする
** TODO 使用できないシチュエーションについて調べる :INPUT:
...
** TODO window-toggle-side-windows(C-x w s)で削除されてしまう
ウィンドウが削除されたらキーボードも削除するようにしたので当然。
残すオプションも追加する？

** TODO ウィンドウにno-other-windowを設定する
C-x oで仮想キーボードに移りたくない。

** TODO 少なくともread-event系では物理キーに対する修飾キーロックできる
...
** TODO unread-command-eventsをうまく使えないか :INPUT:
...
フォーカス移動が他の悟られないなどのメリットはあるだろうか。

** TODO 入力対象フレームの割り出し処理を改善する :INPUT:
...
** TODO Fnキーでレイアウトをごっそり切り替える :INPUT:
** TODO SVGキーボードスタイルを追加する
** TODO Emacsのオンスクリーンキーボードとして使えるようにする
** TODO 押したらサイズが大きくなるというのはどうだろう
...
** SOMEDAY フレームパラメータalphaを指定してみる(半透明にする)
狭い画面で使うので後ろが見えた方が良い。
うーん、Windowsでは効くけどAndroidでは効かないみたい。
それならいいや。そのうち対応したら。

** SOMEDAY タイトルバーにヘッダーラインを使う
